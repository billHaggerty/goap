...
main() {
  ...
  SimpleState origin = new SimpleState()
    ..facts = {"gold": 10, "lumber": 0, "axe": false, "improvedAxe": false};

  var goal = new LumberjackDream();
  ApplyAction<SimpleState> apply = (SimpleState? state) {
    state!.facts!["gold"] -= 10;
    state.facts!["axe"] = true;
  };

  var buyAxe = LumberjackAction("BuyAxe", apply,
  ...
  var chopWoodBetter = new LumberjackAction("ChopWoodBetter",
      (SimpleState? state) {
    state!.facts!["lumber"] += 10;
  },
      prerequisite: (SimpleState? state) =>
  ...
  var solution = planner.plan(origin, goal,
      [buyAxe, improveAxe, chopWood, chopWoodBetter, sellWood, stealGold],
      previouslyFailedAction: stealGold, countdownFailedActionAvoidance: -1);
  solution.then((solution) {
    print(solution!.length);
    for (LumberjackAction action in solution as Iterable<LumberjackAction>) {
      print(origin);
      print(action);
